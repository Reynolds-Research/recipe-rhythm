...
| `prep_time_minutes` | `integer` nullable | **Added 2026-04-27** via [PRD-002 P0.4 migration](../supabase/migrations/20260427000001_vault_prep_time.sql). Estimated minutes of active prep + cook time. `NULL` = unknown. CHECK constraint: `prep_time_minutes IS NULL OR prep_time_minutes > 0`. Written by the recipe-add form via the `PREP_TIME_BUCKETS` chip picker (storedValue per bucket: 15 / 30 / 60 / 90) or by the `analyzeRecipe` AI when it can estimate from the source. The chip picker round-trips via `bucketForMinutes` in `src/lib/constants.js`. Will drive the prep-time badge in BrainstormMode and (paired with `household_preferences` in Phase 3) the prep-time scoring penalty in `src/lib/recommendations.js`. |
...
| [`20260427000001_vault_prep_time.sql`](../supabase/migrations/20260427000001_vault_prep_time.sql) | 2026-04-27 | PRD-002 P0.4: adds `vault.prep_time_minutes` (integer, nullable, CHECK > 0). Populated by the chip-picker buckets in the recipe-add form or the `analyzeRecipe` AI prompt; existing rows default to `NULL` (no backfill). RLS already covers the new column via the existing owner-scoped vault policies. |
| [`verify_20260427_prep_time.sql`](../supabase/migrations/verify_20260427_prep_time.sql) | 2026-04-27 | Read-only verification queries for the prep-time migration: column shape, CHECK constraint, comment, smoke-count of unset/set rows. |
