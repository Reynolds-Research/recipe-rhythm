...
-- for a recipe. Nullable (NULL = unknown). CHECK enforces positive integers.
-- Existing rows default to NULL; the analyzeRecipe AI prompt is extended in
-- the same PR to estimate prep time when it can, so newly-added recipes
...
--      max_prep_time_minutes cap. Phase 2 P0.4 ships only the column
--      and form/AI plumbing; the scoring changes land in P0.5.
--
